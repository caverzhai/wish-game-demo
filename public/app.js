...
'zh-TW': {
  ...
  ar: {
    coinUnit: 'عملات', platformTitle: '自由试用版', platformDesc: '每人送一万。随便玩，可以押红押绿，也可以买彩票（最高1中5000），保险+押红绿，月赚27.5%。纯属娱乐。想接着玩找你的推荐人。', appTitle: 'بركة الأمنيات', loginTip: 'اتصل بالمحفظة للبدء. روابط الدعوة تربط المُحيل تلقائياً.', connectWallet: 'اتصال بالمحفظة', demoEnter: 'لا توجد محفظة؟ ادخل كزائر', logout: 'خروج',
    dockHome: '游戏', dockBbs: '聊天', dockIns: '包赔', dockMe: '提现', disclaimerTitle: 'بيان جاد', disclaimerAgree: 'لقد قرأت وأوافق', announcement: 'إعلان', publishAnnouncement: 'نشر', charityApplyBtn: 'Relief Application', charityName: 'Name', charityGender: 'Gender', charityPhoto: 'Photo', charityCountry: 'Country', charityCity: 'City', charityHelpType: 'Help Needed', charityReason: 'Reason', charityAmount: 'Requested Amount', charityProof: 'Proof', charityProofUpload: 'Upload Proof', charitySubmit: 'Submit', charityGoalNote: 'Goal = Requested x 5', charityDonate: 'Donate', charityDonateNow: 'Donate Now', charitySupport: 'Support', charityOppose: 'Oppose', charityVoteNote: 'Only users with insurance nodes can vote. One vote per project, cannot be changed.', charityComments: 'Comments', charityCommentPh: 'Write a comment...', charitySend: 'Send', charityRaised: 'Raised', charityGoal: 'Goal', charityRequested: 'Requested', charityNoProjects: 'No relief applications yet', charityHowToPlay: 'Help Needed', charityBack: 'Back', charityDonated: 'Donated', charityDonor: 'Donor', charityImgTooLarge: 'Image too large, max 2MB', charityPhotoRequired: 'Photo is required', charityPhotoUploadFail: 'Photo upload failed', charityProofUploadFail: 'Proof upload failed', charitySubmitted: 'Submitted successfully!', charitySubmitFail: 'Submission failed', charityError: 'Error', charityLoadFail: 'Failed to load project', charityAmountInvalid: 'Please enter a valid amount', charityDonateSuccess: 'Donation successful!', charityVoteSuccess: 'Vote recorded!', charityCommentSuccess: 'Comment posted!',
...
async function demoEnter(accountNum) {
  $('loginErr').textContent = '';
  try {
    const body = {};
    if (accountNum && accountNum >= 1 && accountNum <= 100) body.accountNum = parseInt(accountNum, 10);
    const u = await api('/demo-login', body);
    ...
    localStorage.setItem('demoAccountNum', u.accountNum || accountNum || '');
    enterMain();
  } catch (e) { $('loginErr').textContent = e.message; }
}
async function demoRandomLogin() { await demoEnter(); }
async function demoLoginWithInput() {
  const input = $('demoAccountInput'); const val = parseInt(input.value, 10);
  if (!val || val < 1 || val > 100) { $('loginErr').textContent = '请输入 1-100 之间的账号编号'; return; }
  await demoEnter(val);
}
function checkCachedDemoAccount() {
  const cached = localStorage.getItem('demoAccountNum');
  if (cached && $('demoAccountInput')) {
    $('demoAccountInput').value = cached;
    const hint = $('demoCachedHint');
    if (hint) { hint.style.display = 'block'; hint.textContent = '上次使用的账号: U' + String(cached).padStart(3, '0') + ' (已自动填充)'; }
  }
}
...
function init() {
  ...
  applyI18n();
  $('connectBtn').onclick = connectWallet; if ($('demoLoginBtn')) $('demoLoginBtn').onclick = demoLoginWithInput; if ($('demoRandomBtn')) $('demoRandomBtn').onclick = demoRandomLogin; checkCachedDemoAccount();
